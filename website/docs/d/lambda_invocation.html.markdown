...
~> **NOTE:** If you get a generic 403 `AccessDeniedException` error when invoking an [`aws_lambda_function`] despite your IAM policy's permissions granting `lambda:InvokeFunction` calls against the desired function (e.g. `"arn:aws:lambda:<REGION>:<ACCOUNT_ID>:function:my-function"`), confirm that your IAM policy allows for those calls against the desired versions of the function as well (e.g. `"arn:aws:lambda:<REGION>:<ACCOUNT_ID>:function:my-function:*"`).
